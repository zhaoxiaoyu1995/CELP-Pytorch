...
from CELP_CyCTR.model.ops.modules import MSDeformAttn
from CELP_CyCTR.model.positional_encoding import SinePositionalEncoding
...
class MyCrossAttention(nn.Module):
    ...
    def forward(self, q, k, v, supp_valid_mask=None, supp_mask=None, cyc=True):
        ...
        if supp_mask is not None and cyc == True:
            ...
            # [bs, n], True means matched position in supp
            association = (supp_mask == re_map_mask).to(attn.device)
...
class CyCTransformer(nn.Module):
    ...
    def sparse_sampling(self, s_x, supp_mask, supp_valid_mask):
        ...
        for b_id in range(s_x.size(0)):
            ...
            if num_fg < self.rand_fg_num:
                ...
                re_valid_mask = torch.cat(
                    [supp_valid_mask[b_id][supp_mask_b == True], supp_valid_mask[b_id][bg_select_idx]], dim=0)
                ...
                re_valid_mask = torch.cat(
                    [supp_valid_mask[b_id][fg_select_idx], supp_valid_mask[b_id][supp_mask_b == False]], dim=0)
                ...
                re_valid_mask = torch.cat([supp_valid_mask[b_id][fg_select_idx], supp_valid_mask[b_id][bg_select_idx]],
                                          dim=0)
    ...
    def forward(self, x, qry_masks, s_x, supp_mask):
        ...
        for l_id in range(self.num_layers):
            ...
            if self.use_cross:
                k, sampled_mask, sampled_valid_mask = self.sparse_sampling(s_x, supp_mask_flatten,
                                                                           supp_valid_mask) if self.training or l_id == 0 else (
                k, sampled_mask, sampled_valid_mask)
                v = k.clone()
