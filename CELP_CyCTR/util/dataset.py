...
class ContrastSemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, data_list=None, supp_transform=None,
                 transform=None, mode='train', use_coco=False,
                 use_split_coco=False):
        assert mode in ['train', 'val', 'test']
    ...
    def __getitem__(self, index):
        ...
        target_pix = np.where(label == class_chosen)
        ignore_pix = np.where(label == 255)
        contrast_label = np.zeros_like(label)
        for class_index in label_class:
            contrast_pix = np.where(label == class_index)
            if contrast_pix[0].shape[0] > 0:
                contrast_label[contrast_pix[0], contrast_pix[1]] = 1
        contrast_label[ignore_pix[0], ignore_pix[1]] = 255
        ...
        raw_label = label.copy()
        if self.transform is not None:
            image, label, contrast_label = self.transform(image, label, contrast_label)
            for k in range(self.shot):
                support_image_list[k], support_label_list[k] = self.supp_transform(support_image_list[k],
                                                                                   support_label_list[k])
        ...
        if self.mode == 'train':
            return image, label, s_x, s_y, subcls_list, contrast_label
        else:
            return image, label, s_x, s_y, subcls_list, raw_label, contrast_label
