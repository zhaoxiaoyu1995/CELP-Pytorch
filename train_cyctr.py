...
from CELP_CyCTR.model.CyCTR import CyCTR
from CELP_CyCTR.util import dataset, transform, config, contrast_transform
from CELP_CyCTR.util.util import AverageMeter, intersectionAndUnionGPU, poly_learning_rate
...
def get_parser():
    ...
    parser.add_argument('--config', type=str,
                        default='./CELP_CyCTR/model/cyctr/config/pascal/pascal_split0_resnet50.yaml',
                        help='config file')
    parser.add_argument('opts',
                        help='see ./CELP_CyCTR/model/cyctr/config/pascal/pascal_split0_resnet50.yaml for all options',
...
def main():
    ...
    model = CyCTR(layers=args.layers, shot=args.shot, reduce_dim=args.hidden_dims,
...
def train(train_loader, model, optimizer, transformer_optimizer, epoch, base_lrs):
    ...
    for i, (input, target, s_input, s_mask, subcls, contrast_target) in enumerate(train_loader):
        ...
        loss.backward()
        # nn.utils.clip_grad_norm(model.parameters(), max_norm=5, norm_type=2)
        optimizer.step()
