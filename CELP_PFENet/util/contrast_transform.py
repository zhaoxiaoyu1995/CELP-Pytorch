...
class Compose(object):
    ...
    def __call__(self, image, label, contrast_label):
        for t in self.segtransform:
            image, label, contrast_label = t(image, label, contrast_label)
        return image, label, contrast_label
...
class ToTensor(object):
    ...
    def __call__(self, image, label, contrast_label):
        ...
        contrast_label = torch.from_numpy(contrast_label)
        if not isinstance(label, torch.LongTensor):
            label = label.long()
        if not isinstance(contrast_label, torch.LongTensor):
            contrast_label = contrast_label.long()
        return image, label, contrast_label
...
class Resize(object):
    ...
    def __call__(self, image, label, contrast_label):
        ...
        s_mask = contrast_label
        new_h, new_w = find_new_hw(s_mask.shape[0], s_mask.shape[1], test_size)
        # new_h, new_w = test_size, test_size
        s_mask = cv2.resize(s_mask.astype(np.float32), dsize=(int(new_w), int(new_h)), interpolation=cv2.INTER_NEAREST)
        back_crop_s_mask = np.ones((test_size, test_size)) * 255
        back_crop_s_mask[:new_h, :new_w] = s_mask
        contrast_label = back_crop_s_mask
...
class RandScale(object):
    ...
    def __call__(self, image, label, contrast_label):
        ...
        contrast_label = cv2.resize(contrast_label, None, fx=scale_factor_x, fy=scale_factor_y,
                                    interpolation=cv2.INTER_NEAREST)
        return image, label, contrast_label
...
class Crop(object):
    ...
    def __call__(self, image, label, contrast_label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            contrast_label = cv2.copyMakeBorder(contrast_label, pad_h_half, pad_h - pad_h_half, pad_w_half,
                                                pad_w - pad_w_half,
                                                cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
        raw_label = label
        raw_image = image
        raw_contrast_label = contrast_label
        ...
        contrast_label = contrast_label[h_off:h_off + self.crop_h, w_off:w_off + self.crop_w]
        ...
        while (pos_num < 0.85 * raw_pos_num and crop_cnt <= 30):
            image = raw_image
            label = raw_label
            contrast_label = raw_contrast_label
            if self.crop_type == 'rand':
                h_off = random.randint(0, h - self.crop_h)
                w_off = random.randint(0, w - self.crop_w)
            else:
        ...
        if crop_cnt >= 50:
            ...
            contrast_label = cv2.resize(raw_contrast_label, (self.size[0], self.size[0]),
                                        interpolation=cv2.INTER_NEAREST)
        ...
        if image.shape != (self.size[0], self.size[0], 3):
            ...
            contrast_label = cv2.resize(contrast_label, (self.size[0], self.size[0]), interpolation=cv2.INTER_NEAREST)
...
class RandRotate(object):
    ...
    def __call__(self, image, label, contrast_label):
        if random.random() < self.p:
            ...
            contrast_label = cv2.warpAffine(contrast_label, matrix, (w, h), flags=cv2.INTER_NEAREST,
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, label, contrast_label):
        if random.random() < self.p:
            ...
            contrast_label = cv2.flip(contrast_label, 1)
        return image, label, contrast_label
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, image, label, contrast_label):
        if random.random() < self.p:
            ...
            contrast_label = cv2.flip(contrast_label, 0)
        return image, label, contrast_label
